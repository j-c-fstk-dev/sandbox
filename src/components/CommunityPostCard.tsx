...
import ImageCarousel from './ImageCarousel'; // Mantém o carrossel, pois não está relacionado à data
...
interface CommunityPostCardProps {
  ...
  likes: number;
  comments: number;
  date: string; // Continua como string, mas o conteúdo será "2 hours ago"
}
...
export default function CommunityPostCard({
  ...
  likes,
  comments,
  date // Recebe a string de data diretamente
}: CommunityPostCardProps) {

  // REMOVEMOS A LÓGICA DE FORMATAÇÃO DE DATA AQUI
  ...
  const renderContent = () => {
    switch (type) {
      case "image":
        return (
          <div className="space-y-3" data-oid="bhiyd3g">
            <p className="text-neutral-text-dark" whitespace-pre-wrap data-oid="kvj_lho">{content}</p>
            {image &&
      ...
      case "carousel":
        return (
          <div className="space-y-3" data-oid="carousel-post-content">
            <p className="text-neutral-text-dark" whitespace-pre-wrap data-oid="carousel-text">{content}</p>
      ...
      default:
        return <p className="text-neutral-text-dark" whitespace-pre-wrap data-oid="7a_k694">{content}</p>;
    }
  };
